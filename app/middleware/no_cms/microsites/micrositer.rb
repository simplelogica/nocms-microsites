class NoCms::Microsites::Micrositer
  ...
  def replace_host_for response, request, microsite, headers, status
    ...
    response.each do |body|
      ...
      body_change = body.gsub(/#{Settings.host}\/#{microsite.root_path}/, '/')
      body_change = body_change.gsub(/#{Settings.host}\//, '/')
      Rails.logger.info(">>>> Removing microsite root path #{microsite.root_path} from response")
      body_change = body_change.gsub(/#{microsite.root_path}/, '/')
      body_change = body_change.gsub("href=\"'#{microsite.root_path}\"", '/')
      ...
      if root_path_last_char == '/'
        ...
        body_change = body_change.gsub("href=\"#{root_path_without_last_slash}\"", 'href="/"')
      end
